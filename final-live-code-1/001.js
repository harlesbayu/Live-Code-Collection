...
function cariPenjahat(arr){
    ...
    for(var i = 0; i < arr.length; i++){
        ...
        } else if(arr[i] === 'Villain' && jebakan !== 0){
            person.push(arr[i])
        }
    }

    if(jebakan === 0){
        return 'Ooops, you die. You got ' + person.length + ' villains'
    } else {
        return 'Nice work, detective! You got all villains: ' + person.length
    }
}

console.log(cariPenjahat(['*', '*', '@', '*', 'Villain', '*',  '@', 'Villain']))
console.log(cariPenjahat(['*', '*', '@', '*', '@' ,'Villain', '*', 'Villain', 'Villain', '@']))
console.log(cariPenjahat(['*', '*', '@', '*', '@' ,'Villain', '*', 'Villain',  '@', 'Villain']))
