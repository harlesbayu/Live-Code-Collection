...
function minDistanceBetweenGreatest(arr) {
  ...
  for(var i = 0; i < bigValues.length; i++){
    for(var j = 0; j < bigValues.length; j++){
      if(bigValues[j] < bigValues[j+1]){
        var sorting = bigValues[j]
        bigValues[j] = bigValues[j+1]
        bigValues[j+1] = sorting
      }
    }
  }
  
  var bigValue = bigValues[0]
  // console.log(bigValue)
  var newArr = []
  for(var k in handle){
    if(bigValue === handle[k]){
  ...
  if(newArr.length === 1){
    return 0
  } else if(newArr.length === handle.length){
    return 1
  }

  var result = []
  for(var x = 1; x < newArr.length; x++){
    result.push(newArr[x]-newArr[x-1])
  }
