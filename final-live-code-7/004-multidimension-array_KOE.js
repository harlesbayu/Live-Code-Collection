...
function digitsGrouper(numbers) {
  ...
  for (var i=0; i < numbers.length ; i++){
    if ((numbers[i].toString()).length ==1 ){
      satuDigit.push(numbers[i])
    }
    else if ((numbers[i].toString()).length ==2){
      duaDigit.push(numbers[i])
    }
    else if ((numbers[i].toString()).length ==3){
      tigaDigit.push(numbers[i])
    }
    else {
      empatDigit.push(numbers[i])
    }
  }
  hasilFinal.push(satuDigit)
  hasilFinal.push(duaDigit)
  hasilFinal.push(tigaDigit)
  hasilFinal.push(empatDigit)
